(function (root, factory) {
   ...
   *                              error
   */
  QCodeDecoder.prototype.decodeFromCamera = function (videoElem, cb, done, err, once) {
	var scope = (this.stop(), this);
  
	if (!this.hasGetUserMedia())
	  cb(new Error('Couldn\'t get video from camera'));
		  var parent = document.querySelector('scanner');
	navigator.mediaDevices.getUserMedia ({audio:false, video:{ facingMode: "environment" , width:parent.offsetWidth, height:parent.offsetHeight}})
	.then(function (stream) {
  
	  videoElem.srcObject = stream;
	  scope.videoElem = videoElem;
	  scope.stream = stream;
	  scope.videoDimensions = true;
  ...
  QCodeDecoder.prototype.setSourceId = function (sourceId) {
	if (sourceId)
	  ...
	  this.videoConstraints.video = { facingMode: { exact: "environment" } };
